...
export const DecryptPasswordsAndValidate: any = createParamDecorator(
  async (data: unknown, ctx: ExecutionContext): Promise<any> => {
    ...
    const decryptedPassword = CryptoJS.AES.decrypt(encryptedDto.password, process.env.PASSWORD_DECRYPTION_KEY ?? "").toString(CryptoJS.enc.Utf8);
    const decryptedConfirmPassword = CryptoJS.AES.decrypt(encryptedDto.confirm_password, process.env.PASSWORD_DECRYPTION_KEY ?? "").toString(CryptoJS.enc.Utf8);

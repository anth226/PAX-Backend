...
import { ACCESS_TOKEN, PRE_AUTH_TOKEN, REFRESH_TOKEN } from 'src/utils/constants';
import { HasNextPage } from 'src/@types/common';
...
export class AuthService {
  ...
  constructor(
    ...
    private readonly loginAttemptModel: Repository<LoginAttemptEntity>,
    @InjectRepository(TwoFactorMethodEntity)
    private readonly towFaModel: Repository<TwoFactorMethodEntity>,
  ...
  async login(req: Request, userData: any, ip: string, ua: string, method: string) {
    try {
      userData.password = this.decryptCryptoJS(userData.password)
      ...
      if (!user.isActivated) {
        throw new HttpException(
          this.i18n.translate('common.auth.inactive'),
          HttpStatus.BAD_REQUEST,
        );
      }
      await this.removeLimit(user.email, ip);
      const hasNextPage = await this.checkHasNextPage(req, 'password', new UserDto(user));
  ...
  async validateUser(userData: any, ip: string): Promise<any> {
    try {
      ...
      if (!user) {
        throw new HttpException(
          {
            message: this.i18n.translate('common.auth.invalid_email', {
      ...
      if (!isPasswordEquals) {
        ...
        throw new HttpException(
          {
            message: this.i18n.translate('common.auth.incorrect_password'),
      ...
      if (error?.msBeforeNext) {
        throw new HttpException(
          {
            message: this.i18n.translate('common.auth.too_many_requests'),
  ...
  async limitLogin(username: string, ip: string) {
    ...
    if (softLimitResponse && softLimitResponse.consumedPoints > softLockoutTries) {
      throw new HttpException(
        {
          message: this.i18n.translate('common.auth.too_many_requests'),
          msBeforeNext: softLimitResponse.msBeforeNext,
        },
        429,
      );
    }
    if (hardLimitResponse && hardLimitResponse.consumedPoints > hardLockoutTries) {
      throw new HttpException(
        {
          message: this.i18n.translate('common.auth.too_many_requests'),
          msBeforeNext: hardLimitResponse.msBeforeNext,
        },
        429,
      );
    }
  ...
  async generateToken(user: any) {
    ...
    const accessToken = this.jwtService.sign(payload, {
      secret: process.env.JWT_ACCESS_SECRET || "key",
      expiresIn: process.env.JWT_ACCESS_EXPIRES_IN || '10m',
    });
    const refreshToken = this.jwtService.sign(payload, {
      secret: process.env.JWT_REFRESH_SECRET || "key",
  ...
  async savePreAuthToken(req: Request, payload: any) {
  ...
  async saveToken(
    ...
    req.res.clearCookie(PRE_AUTH_TOKEN)
    const domain = process.env.MAIN_DOMAIN || 'paxtraining.com';
  ...
  async generateOtpMail(email: string) {
    ...
    if (otpExists) {
      const diff: number = Number(otpExists.resendIn) - currentTime;
      if (diff > 0) {
        throw new HttpException(
          {
            message: this.i18n.translate('common.auth.already_otp', {
              args: { time: this.formatTimeDuration(diff) },
            }),
    ...
    await this.mailService.sendMailCode(email, otpCode, user.nameFirst);
    if (otpExists) {
    ...
    const diff: number = Number(resendIn) - currentTime;
    return {
      resendIn: Math.floor(diff / 1000),
    };
  }
  ...
  async verifyOtpMail(
    ...
    try {
      ...
      if (!user) {
        throw new HttpException(
          this.i18n.translate('common.auth.invalid_email', { args: { email } }),
          HttpStatus.BAD_REQUEST,
        );
      }
      const otp = await this.otpModel.findOneBy({ email: methodDetail });
      if (!otp) {
        throw new HttpException(
          this.i18n.translate('common.auth.no_otp'),
          HttpStatus.BAD_REQUEST,
        );
      }
      ...
      if (otp.code != code) {
        throw new HttpException(
          this.i18n.translate('common.auth.invalid_otp'),
          HttpStatus.BAD_REQUEST,
        );
      }

      const currentTime = new Date().getTime();
      const diff: number = Number(otp.expiresIn) - currentTime;
      if (diff < 0) {
        // code expired
        throw new HttpException(
          this.i18n.translate('common.auth.otp_expired'),
          HttpStatus.BAD_REQUEST,
        );
      }
      ...
      const hasNextPage = await this.checkHasNextPage(req, '2fa', new UserDto(user));
      if (hasNextPage) return hasNextPage;
      ...
      try {
        ...
        if (tlError?.msBeforeNext) {
          throw new HttpException(
            {
              message: this.i18n.translate('common.auth.too_many_requests'),
              msBeforeNext: tlError.msBeforeNext,
            },
            429,
          );
        }
  ...
  async requirePassReset(
    ...
    const current_password = this.decryptCryptoJS(body.current_password)
    const confirm_password = this.decryptCryptoJS(body.confirm_password)
    const new_password = this.decryptCryptoJS(body.new_password)
    if(new_password!==confirm_password) {
        throw new BadRequestException({
            message: this.i18n.translate('common.auth.not_matched', {args: {
                value1: "New password",
                value2: "Confirm password"
            }}),
        });
    }
    await this.updatePassword(userDto?.email, current_password, new_password, ip);
    const hasNextPage = await this.checkHasNextPage(req, 'reset-password', userDto);
    if (hasNextPage) return hasNextPage;
  ...
  async updatePassword(email: string, current_password: string, new_password: string, ip: string) {
    ...
    return this.userModel.update({ id: user.id }, { password: hashPassword, requirePassReset: false });
  }

  async getTwoFactorMethodById(methodId: number, userId: number) {
    const twoFa = await this.towFaModel.findOneBy({id: methodId, user: {id: userId}})
    if(!twoFa) {
        throw new HttpException(this.i18n.translate('common.auth.invalid_two_fa_method'), HttpStatus.BAD_REQUEST);
    }
    return twoFa
  }

  async getUserTwoFactorMethodDetails(userId: number): Promise<{ methodType: string, methodDetail: string }[]> {
    const methods = await this.towFaModel.find({
      where: { user: { id: userId } },
    });
    return methods.map((method) => ({
        id: method.id,
        methodType: method.methodType,
        methodDetail: method.formattedMethodDetail(),
    }));
  }

    async checkHasNextPage(req: Request, currentPage: string, user: UserDto) {
        const response: HasNextPage  = {
            statusCode: HttpStatus.OK,
            message: this.i18n.translate('common.auth.user_information'),
            nextPage: null,
        }
        if (currentPage == 'password' && user?.isTwoFactorAuthenticationEnabled) {
            ...
            response.nextPage = '2fa';
            response.methods = await this.getUserTwoFactorMethodDetails(user.id)
        } else if (currentPage != 'reset-password' && user?.requirePassReset) {
            const payload = { id: user.id, email: user.email };
            this.savePreAuthToken(req, payload);
            response.nextPage = 'reset-password';
        } else if (!user?.hasAcceptedLatestTOS) {
            const payload = { id: user.id, email: user.email };
            this.savePreAuthToken(req, payload);
            response.nextPage = 'agreement';
        }
  ...
  async activateAccount(activationLink: string): Promise<any> {
    ...
    if (!user) {
      throw new HttpException(
        this.i18n.translate('common.auth.invalid_activation_link'),
        HttpStatus.BAD_REQUEST,
      );
    }
    user.isActivated = true;
  ...
  decryptCryptoJS(value: string) {
    try {
      ...
      if (!decryptedValue) {
        throw new BadRequestException(
          this.i18n.translate('common.auth.invalid_format', { args: { value } }),
        );
      }
  ...
  async checkResetPassword(resetLink: any) {
    ...
    const currentTime = new Date().getTime();
    const diff: number = Number(user.resetLinkExpiresIn) - currentTime;
    if (diff <= 0) {
  ...
  async newResetPassword(password: string, confirm_password: string, resetLink: string) {
    ...
    if (decryptedPassword !== decryptedConfirmPassword) {
      throw new BadRequestException(
        this.i18n.translate('common.auth.not_matched', {
          args: { value1: 'Password', value2: 'Confirm Password' },
        }),
      );
    }
    const user = await this.userModel.findOneBy({ resetLink });
  ...
  async phoneVerifyService(
    ...
    try {
      ...
      if (!user) {
        throw new HttpException(
          this.i18n.translate('common.auth.invalid_phone'),
          HttpStatus.BAD_REQUEST,
        );
      }
      const otp = await this.otpModel.findOneBy({ phone: methodDetail });
      if (!otp) {
        throw new HttpException(
          this.i18n.translate('common.auth.invalid_phone'),
          HttpStatus.BAD_REQUEST,
